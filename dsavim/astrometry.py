...
class SourceCatalog:
    ...
    def __init__(self, image: Image, catalogpath: str = ""):
    ...
    def load(self, sparse=True):
        ...
        if (not os.path.exists(self._catalogpath) or 
                os.path.getmtime(self._catalogpath) < os.path.getmtime(self._filepath)):
            self.generate()
    ...
    def _write_empty_catalog(self):
        with open(self._catalogpath, 'w', encoding='utf-8') as f:
            f.write((
                f"# Gaussian list for {self._filepath}\n"
                "# Generated by PyBDSM version 1.10.1\n"
                f"# Reference frequency of the detection (\"ch0\") image: {self._f0}\n"
                "# Equinox : 2000.0\n"
                "\n"
    ...
    @catalogpath.setter
    def catalogpath(self, value: str) -> None:
