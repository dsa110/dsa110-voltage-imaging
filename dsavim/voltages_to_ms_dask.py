...
def generate_correlate_component(
        dispersion_measure: float, ntint: int, corr_ch0: dict,
        npol: int) -> "Callable":
    """Generate a correlator function."""

    def correlate(vfile, ncorrfiles):
        """Correlate a file."""
        if ncorrfiles.get() > 2:
...
def generate_uvh5_component(
        ...
        start_offset: int, end_offset: int) -> "Callable":
    """Generate a uvh5 writer."""

    def write_uvh5(corrfile, ncorrfiles):
