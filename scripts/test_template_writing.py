...
def test_template_writing():
	...
	correlation_parameters = get_correlation_parameters(NTINT, NFINT, START_OFFSET, STOP_OFFSET)
	correlated_paths = {}
	...
	correlated_paths_string = '\n'.join([f'{key} {value}' for key, value in correlated_paths.items()])
...
def get_correlation_parameters(ntint: int, nfint: int, start_offset: int, stop_offset: int):
	input_params = load_params(PARAMFILE)
	deltat_ms = ntint*input_params['deltat_s']*1e3
	nants = len(input_params['antennas'])
	corr_dir = input_params['corrdir']
